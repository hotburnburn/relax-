    ...
    <header className="game-header">
      <div className="header-top">
        ...
        <div className="header-actions">
          ...
          >
            <Undo2 size={18} />
            <span className="btn-label">{t.header.undoLabel}</span>
          </button>
          ...
          >
            <Redo2 size={18} />
            <span className="btn-label">{t.header.redoLabel}</span>
          </button>

          <button 
            className="action-btn icon-btn hint-btn" 
            onClick={onHint}
            title={t.header.hintTooltip}
          >
            <Lightbulb size={18} className="hint-bulb" />
            <span className="btn-label">{t.header.hintLabel}</span>
          </button>
          ...
          <button 
            ...
            title={isMuted ? t.header.unmuteTooltip : t.header.muteTooltip}
          >
          ...
          <button 
            className="action-btn icon-btn lang-btn" 
            onClick={(e) => { e.stopPropagation(); onToggleLanguage(); }}
            title={lang === 'zh' ? 'Switch to English' : '切换为中文'}
          >
            <Languages size={18} />
            <span className="btn-label">{lang === 'zh' ? 'EN' : '中文'}</span>
          </button>
      ...
      <div className="header-bottom">
        ...
        <div className="difficulty-container">
          <label htmlFor="difficulty-select">{t.header.difficultyLabel}</label>
          <select 
          ...
          >
            <option value={1}>{t.header.difficultyEasy}</option>
            <option value={2}>{t.header.difficultyMedium}</option>
            <option value={4}>{t.header.difficultyHard}</option>
          </select>
        </div>
        ...
        <div className="scoreboard">
          <div className="score-stat">
            <span className="stat-label">{t.header.scoreLabel}</span>
            <span className="stat-value highlight">{score}</span>
          </div>

          <div className="score-stat">
            <span className="stat-label">{t.header.movesLabel}</span>
            <span className="stat-value">{movesCount}</span>
          </div>

          <div className="score-stat">
            <span className="stat-label">{t.header.timeLabel}</span>
